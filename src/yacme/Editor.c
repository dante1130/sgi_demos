...
void YACME_init(int x, int y, int w, int h, int dim, float **list,
	...
	*/
	for (i = 0; i < 4; i++) {
		for (userpoint = userPoint[i]; userpoint != NULL;
		     userpoint = userpoint->next)
...
static void Key(unsigned char key, int x, int y) {
	...
	switch (key) {
		...
		case 8:   /* Delete */
			if (!curPoint) return;
			previousPoint =
			    (curPoint->last ? curPoint->last : userPoint[curCmpnt]);
...
/*      Special
/*---------------------------------------------------------------------------*/
static void Special(int key, int xx, int yy) {
	int x, y, minx, maxx;
	float dx, dy;
	...
	minx = (int)curPoint->last->x;
	minx++;
	maxx = (int)curPoint->next->x;
	maxx--;
...
/*	YACME_menuFunc
/*---------------------------------------------------------------------------*/
static void YACME_menuFunc(int item) {
	switch (item) {
		case CLOSE_ITEM:
			break;
...
static void Redraw(void) {
	...
	if (update) {
		...
		if (newmapCB && (YACME_refresh == CONTINUOUS)) (*newmapCB)();
		update = 0;
	}
	...
	*/
	...
	cmpnt = priority[0];
	for (i = 0; i < 4; i++, cmpnt = priority[i])
		if (DrawCurve[cmpnt]) {
			switch (cmpnt) {
				case 3:
					glColor3f(1., 1., 1.);
			...
			for (j = 0; j < MAXNDX; j++) {
				...
				glVertex2f((float)(j + 1),
				           (float)MAXNDX * TabCmpnt(cmpnt, j + 1));
				glEnd();
			}
		}
...
#define DIMPOINT (2. * LUTRATIO)
	...
	*/
	glColor3f(0., 0., 0.);
	sprintf(string, "%.5d: rgba %.3f %.3f %.3f %.3f", ndx, TabCmpnt(0, ndx),
	        TabCmpnt(1, ndx), TabCmpnt(2, ndx), TabCmpnt(3, ndx));
...
void YACME_pick(int mousex, int mousey, PickObject *obj) {
	...
	*/
	...
	pickrad = 3. * LUTRATIO;
	cmpnt = priority[3];
	for (i = 3; i >= 0; i--, cmpnt = priority[i])
		if (DrawCurve[cmpnt]) {
			...
			while (upoint != NULL) {
				/*
				        On cherche le POINT
				*/
				if (ABS(y - upoint->y) < pickrad &&
				    ABS(x - upoint->x) < pickrad) {
					obj->upoint = upoint;
					obj->cmpnt = cmpnt;
					obj->type = POINT;
					return;
			...
			/*
			    ...
			    (On considere la precision suffisante pour le picking suivant
			   l'axe X) Remarque : on ne sort pas en ce cas, car la priorite est
...
void YACME_update(int cmpnt, UserPoint *upoint) {
	...
	if (upoint->last != NULL) {
		...
		GetPolynome(upoint->last->mode, upoint->last->x, upoint->last->y, t1,
		            upoint->x, upoint->y, t0, upoint->last->polynome);
	...
	if (upoint->next != NULL) {
		...
		GetPolynome(upoint->mode, upoint->x, upoint->y, t0, upoint->next->x,
		            upoint->next->y, t1, upoint->polynome);
...
/*			MovePoint
/*---------------------------------------------------------------------------*/
int MovePoint(int mousex, int mousey) {
...
void GetPolynome(int mode, float a0, float b0, float t0, float a1, float b1,
	...
	switch (mode) {
		...
		case POLYNOMIAL:
			/*
			            we want a polynom P(x) = Ax^3 + Bx^2 + Cx + D
			            so that : P(a0) = b0, P'(a0) = t0, P(a1) = b1, P'(a1) =
			   t1 i.e.
			...
			*/
			...
			for (i = 0; i < 4; i++)
				coeff[i] = INV_mat[i][0] * b0 + INV_mat[i][1] * b1 +
				           INV_mat[i][2] * t0 + INV_mat[i][3] * t1;
			break;
	}
}
